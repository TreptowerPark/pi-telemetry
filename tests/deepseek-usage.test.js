...
test("classifies current DeepSeek V4 model families", () => {
  assert.equal(getDeepSeekPricingFamily("deepseek-v4-flash"), "flash");
  assert.equal(getDeepSeekPricingFamily("deepseek-v4-flash-vision-exp"), "flash");
  assert.equal(getDeepSeekPricingFamily("deepseek-v4-pro"), "pro");
});
...
test("calculates Flash off-peak cost and cache savings", () => {
  ...
  assert.ok(Math.abs(result.cost - 0.887) < 1e-12);
  assert.ok(Math.abs(result.cacheSaved - 0.213) < 1e-12);
});

test("calculates Flash peak cost and cache savings", () => {
  const result = calculateDeepSeekUsageCost(
    "deepseek-v4-flash",
    { cacheHitTokens: 1_000_000, cacheMissTokens: 1_000_000, outputTokens: 1_000_000 },
    Date.parse("2026-09-14T02:00:00Z"),
  );
  assert.ok(result);
  assert.equal(result.peak, true);
  assert.ok(Math.abs(result.cost - 1.774) < 1e-12);
  assert.ok(Math.abs(result.cacheSaved - 0.426) < 1e-12);
});

test("calculates Pro pricing separately", () => {
  const result = calculateDeepSeekUsageCost(
    "deepseek-v4-pro",
